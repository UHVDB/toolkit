process INTERPROSCAN_INTERPROSCAN {
    ...
    containerOptions "${(workflow.containerEngine == 'singularity' || workflow.containerEngine == 'apptainer') ? "--bind ${db}/data:/opt/interproscan/data" : "-v ${db}/data:/opt/interproscan/data"}"
    ...
    stub:
    """
    echo "" | gzip > ${meta.id}.interproscan.tsv.gz
    """
}
