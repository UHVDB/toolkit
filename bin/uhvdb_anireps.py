...
def load_metadata(classify_tsv, completeness, uhvdb_metadata, clusters):
    classify_df = pl.concat([
        pl.read_csv(classify_tsv, separator='\t', ignore_errors=True, null_values=['NA'], columns=['uhvdb_id', 'completeness_method', 'contig_length', 'proviral_length', 'viral_genes']),
        pl.read_csv(uhvdb_metadata, separator='\t', ignore_errors=True, null_values=['NA'], columns=['uhvdb_id', 'completeness_method', 'contig_length', 'proviral_length', 'viral_genes'])
    ], how='vertical_relaxed')
    completeness_df = pl.concat([
        pl.read_csv(completeness, separator='\t', columns=['uhvdb_id', 'aai_expected_length'], ignore_errors=True),
        pl.read_csv(uhvdb_metadata, separator='\t', columns=['uhvdb_id', 'aai_expected_length'], ignore_errors=True)
    ])
    ...
    mine_report = (
        ...
        classify_df
            ...
            .filter(
                (pl.col('uhvdb_id').is_in(clusters.keys()))
            )
            ...
            .with_columns([pl.col('uhvdb_id').replace_strict(clusters, default=None).alias('cluster_id')])
    )
...
def main(args=None):
    ...
    cluster_reps = (
        mine_report
            .filter(pl.col('cluster_id').is_in(singleton_clusters))['uhvdb_id', 'cluster_id']
    )
    ...
    linear_cluster_reps = (
        mine_report_metrics
            ...
            .sort(['length_diff', 'uhvdb_id'], descending=[True, False])
            .group_by('cluster_id', maintain_order=True)
            .first()['uhvdb_id', 'cluster_id']
    )
    ...
    # 5. Output all representatives
    cluster_reps[['uhvdb_id']].write_csv(args.output_reps)
    
    # 5.2 Output new representatives
    if args.cluster_level == 'species':
        uhvdb_ids = set(pl.read_csv(args.uhvdb_metadata, separator='\t', ignore_errors=True, null_values=['NA'], columns=['species_rep'])['species_rep'])
        (
        cluster_reps
            .filter(~pl.col('uhvdb_id').is_in(uhvdb_ids))
            [['uhvdb_id']]
            .write_csv(args.output_new_reps, separator='\t', include_header=False)
        )
    elif args.cluster_level == 'genomovars':
        uhvdb_ids = set(pl.read_csv(args.uhvdb_metadata, separator='\t', ignore_errors=True, null_values=['NA'], columns=['genomovar_rep'])['genomovar_rep'])
        (
        cluster_reps
            .filter(~pl.col('uhvdb_id').is_in(uhvdb_ids))
            [['uhvdb_id']]
        ...
        cluster_reps[['uhvdb_id']].write_csv(args.output_new_reps)

    # 6. Output cluster information
    (
        mine_report_metrics
            [['uhvdb_id', 'cluster_id', 'num_seqs', 'length', 'median_length', 'aai_expected_length', 'viral_genes', 'max_viral_genes', 'completeness_method']]
